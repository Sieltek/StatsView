...
class MatchHisto extends React.Component {
    ...
    async getSummonerMatchInfo(i, gameId) {
        let url = "localhost:4000/getMatch/" + gameId
    ...
    async getChampId(id, id_champ) {
        ...
        let url = "localhost:4000/getchamp"
        let res = await fetch(url, { method: "GET" })
    ...
    async getSummonerMatchHisto() {
        if (this.props.accountId) {
            var champImg = []
            let url = "localhost:4000/getMatchList/" + this.props.accountId + "/endIndex=" + this.state.endIndex + "&beginIndex=" + this.state.beginIndex
            let res = await fetch(url, { method: "GET" })
